...
::: warning BREAKING
This release comes with a major rewrite of the Prisma-AppSync Client API and several breaking changes. Please make sure to read the details below before upgrading.
:::

## Highlights

- **🪓 Breaking**: Prisma-AppSync was rewritten from the ground (simplified API, more opinionated, fully typed, better DX and logs, hooks, fine-grained control, etc).
- **🎉 New**: Interactive scaffolding CLI to quickly start new Prisma-AppSync projects, accessible from a single `npx create-prisma-appsync-app` command. It can also plug into existing projects already using Prisma.
...
- **🎉 New**: Documentation website and contribution guide on [prisma-appsync.vercel.app](https://prisma-appsync.vercel.app).
- **🐞 Improvement**: Dozen of improvements, bug fixes and new Prisma features added (atomic operations, order by relations, case sensitivity, etc).
- **💛 Github sponsor**: Enjoy using Prisma-AppSync? Please consider [support me](https://github.com/sponsors/maoosi).
