...
try {
    ...
    // build create-app
    console.log(chalk.blue('\nBuild :: Create app\n'))
    await $`esbuild packages/create-app/src/index.ts --bundle --platform=node --external:fsevents --external:_http_common --outfile=dist/create-app.js`

    // build appsync-server
    console.log(chalk.blue('\nBuild :: AppSync-server\n'))
    await $`esbuild packages/appsync-server/src/index.ts --bundle --format=cjs --keep-names --platform=node --external:fsevents --external:_http_common --outfile=dist/appsync-server/index.js`
