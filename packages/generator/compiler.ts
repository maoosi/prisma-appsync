...
export class PrismaAppSyncCompiler {
                ...
                model.fields.forEach((field: DMMF.Field) => {
                    ...
                    if (!field.isGenerated && !this.isIgnored(field)) {
                        fields.push({
                            ...
                            isEditable:
                                !this.isFieldGeneratedRelation(field, model) && !this.isFieldImmutable(field),
    ...
    private isFieldImmutable(searchField: DMMF.Field): boolean {
        ...
        return defaultValue?.name === 'autoincrement' || searchField.isUpdatedAt || ['updatedAt', 'createdAt'].includes(searchField.name)
    }
