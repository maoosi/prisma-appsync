...
async function init() {
    ...
    userConfig.projectName = String(path.join(cwd, userConfig.targetDir).split(/\/|\\/).pop())
    ...
    if (createLocalDevServer) {
        ...
        const concurrentlyCmd = [
            ...
            `"npx nodemon -e prisma,gql,yaml --watch './**/*' --ignore './node_modules' --ignore '**/generated/**' --exec 'npx prisma generate && echo \\'Running a GraphQL API server at http://localhost:4000/graphql\\''"`,
        ]
        ...
        if (!isLocalDevMode) {
            ...
            concurrentlyCmd.push(`"wait-on tcp:4000 && npx nodemon -e ts --watch '../packages/**/*' --exec 'cd ../ && pnpm build && echo \\'Running a GraphQL API server at http://localhost:4000/graphql\\''"`)
    ...
    userConfig.injects.push({
        file: path.join(userConfig.root, 'cdk/src/index.ts'),
        find: /\{\{ projectName \}\}/g,
        replace: userConfig.projectName,
    })
    userConfig.injects.push({
        file: path.join(userConfig.root, 'cdk/src/index.ts'),
        find: /\{\{ relativeGqlSchemaPath \}\}/g,
        replace: path.relative(
            path.join(userConfig.root, 'cdk/src'), 
            path.join(userConfig.root, path.dirname(userConfig.targetSchema), 'generated/prisma-appsync/schema.gql')
        ),
    })
    userConfig.injects.push({
        ...
        replace: path.relative(
            path.join(userConfig.root, 'cdk/src'), 
            path.join(userConfig.root, path.dirname(userConfig.targetSchema), 'generated/prisma-appsync/resolvers.yaml')
        ),
    })
    userConfig.injects.push({
        file: path.join(userConfig.root, 'cdk/src/index.ts'),
        ...
        replace: path.relative(
            path.join(userConfig.root, 'cdk/src'), 
            path.join(userConfig.root, 'handler.ts')
        ),
    })
    userConfig.scripts.push({
        name: 'deploy',
...
async function execUserConfig(userConfig: UserConfig) {
    ...
    // install cdk deps
    await execa(userConfig.packageManager, ['install'], { stdio: 'inherit', cwd: path.join(userConfig.root, 'cdk') })
